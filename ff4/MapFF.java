...
public class MapFF extends Mapper<IntWritable,Vertex, IntWritable,Vertex> {
	...
	IntWritable vid = new IntWritable(), first, last;
	...
	public void map(IntWritable key, Vertex node, Context context) throws IOException, InterruptedException {
		...
		context.getCounter("ff4","SOURCE_E_SATURATED").increment(stsaturated[0]);
		context.getCounter("ff4","SINK_E_SATURATED").increment(stsaturated[1]);
		if (oS>0 && oS == stsaturated[0]) context.getCounter("ff4","LOSE_EXCESS").increment(1);
		if (oT>0 && oT == stsaturated[1]) context.getCounter("ff4","LOSE_EXCESS").increment(1);

		long t2 = System.currentTimeMillis();
		context.getCounter("ff4","TIME_UPDATE_EDGES").increment(t2-t1);

		//	node.checkSSMeet();

		long t3 = System.currentTimeMillis();
		context.getCounter("ff4","TIME_MEET_IN_THE_MIDDLE").increment(t3-t2);
		...
		if (node.nS > 0){
			...
			for (int i=0; i<node.nE; i++){
				...
				vertex.S[0].copy(node.S[(int) (Math.random() * node.nS)]);
			...
			context.getCounter("ff4","EXTEND_SOURCE_E").increment(cnt);
		}

		long t4 = System.currentTimeMillis();
		context.getCounter("ff4","TIME_EXTEND_SOURCE_E").increment(t4-t3);
		...
		if (node.nT > 0){
			...
			for (int i=0; i<node.nE; i++){
				...
				vertex.T[0].copy(node.T[(int) (Math.random() * node.nT)]);
			...
			context.getCounter("ff4","EXTEND_SINK_E").increment(cnt);
		}

		long t5 = System.currentTimeMillis();
		context.getCounter("ff4","TIME_EXTEND_SINK_E").increment(t5-t4);
		context.getCounter("ff4","TIME_MAPPER").increment(t5-t1);
	}
}
